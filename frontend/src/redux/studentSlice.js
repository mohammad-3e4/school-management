...
export const toggleAttendance = createAsyncThunk(
  ...
  async ({ student_id, attendance, typeCase }) => {
    try {
      const requestBody = {
        student_id,
        attendance,
...
export const deleteEntry = createAsyncThunk(
  ...
  async ({ student_id, attendance }, thunkAPI) => {
    const requestBody = {
      student_id,
      attendance,
    };
  ...
  reducers: {
    ...
    setSeacrh: (state, action) => {
      state.students = state.students.filter((student) =>
        student.student_name.includes(action.payload)
      );
    },
  },
...
export const { clearErrors, clearMessage, setSeacrh } = studentSlice.actions;
