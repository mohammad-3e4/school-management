...
const AllStudents = () => {
  ...
  const [deleteStudent, setDeleteStudentId] = useState(null);
  ...
  const thds = [
    selectedClass == "undefined" ? "ID" : "Roll no",
    "Photo",
    "Name",
    "Gender",
    "Parents Name",
  ...
  const handleSearch = (e) => {
    dispatch(setSeacrh(e.target.value));
    if (e.target.value === "") {
      dispatch(getStudents(selectedClass));
    }
  ...
  const handleDelete = (student) => {
     setDeleteStudentId(student)
  };
  return (
    <section className="py-1  w-full m-auto">
      <div className="flex flex-wrap justify-between shadow bg-white py-2 mb-1 w-[98%] m-auto">
      ...
      >
    {deleteStudent && <Confirmation data={deleteStudent} isClose={()=>setDeleteStudentId(null)}/>}
        {loading ? (
          <Loader />
        ) : (
                ...
                {students?.length == 0 ? (
                  ...
                  students?.map((student) => (
                    ...
                    >
                      ...
                      >
                        <FaRegIdCard className="h-5 w-5 text-blue-700 cursor-pointer" />
                      </td>
