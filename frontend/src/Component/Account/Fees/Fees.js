...
import { getFees, clearErrors, clearMessage } from "../../../redux/feesSlice";
...
export default function Fees() {
  ...
  const { loading, error, message, fees } = useSelector((state) => state.fees);
  console.log(fees);
  const dispatch = useDispatch();

  useEffect(() => {
    dispatch(getFees(selectedClass));
    if (error) {
                ...
                fees?.map((student, index) => (
                  <tr key={index}>
                    ...
                    <td className="py-2 item-center text-[12px] border-b-2">
                      {student.due_fees > 0 || student.due_fees == null ? (
                    ...
                    <td className="py-2 text-center text-[12px] border-b-2">
                      {" "}
                      {new Date(student.date_time).toLocaleDateString()}
                    </td>
                    <td className="py-2 text-center text-[12px] border-b-2 ">
                      {" "}
                      <Link to={`/fees/details/${student.student_id}`}>
