...
exports.getClassSubject = catchAsyncErrors(async (req, res, next) => {
  try {
    const {selectedClass}=req.params
    
    console.log(req.params)
    const result = await new Promise((resolve, reject) => {
      db.query(`select * from classes where class_name='${selectedClass}'`, (error, results) => {
        if (error) {
          reject(error);
        } else {
          resolve(results);
